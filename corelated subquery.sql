...
select emp_id,emp_name,gender,salary,dept_id from employee as e where 
...
select e.emp_id, e.emp_name,d.dept_name,e.salary from employee as e join department as d on e.dept_id=d.dept_id where salary>(select avg(salary) from employee where dept_id=e.dept_id);
select * from employee;
select * from department;
...
select dept_id,dept_name from department where dept_id=any(select distinct(dept_id) from employee where salary>70000);
select emp_name,dept_id,salary,gender from employee as e1 where salary=( 
select max(salary) as second_max from employee  where dept_id=e1.dept_id and salary<(select max(salary) from employee where dept_id=e1.dept_id));
